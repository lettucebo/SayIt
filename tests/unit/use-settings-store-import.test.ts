...
const h = vi.hoisted(() => {
  ...
  return {
    ...
    mockClearAzureTokenCache: vi.fn(),
  };
});
...
vi.mock("../../src/lib/azureAuth", () => ({
  clearAzureTokenCache: h.mockClearAzureTokenCache,
  getAzureAccessToken: vi.fn(),
  getAzureScopeForApiKind: vi.fn(() => "scope"),
}));
...
describe("useSettingsStore — exportSettings / importSettings", () => {
  beforeEach(() => {
    ...
    h.mockClearAzureTokenCache.mockClear();
  });
  ...
  describe("exportSettings", () => {
    ...
    it("[P0] excludeSecrets=true 剔除金鑰、保留非敏感設定", async () => {
      h.mockStoreData.set("groqApiKey", "gsk_secret");
      h.mockStoreData.set("azureClientSecret", "secret");
