...
| `set_analytics_enabled` | `plugins/analytics.rs` | useSettingsStore | `enabled: bool` | `()` |
...
Sentry 負責錯誤 / 崩潰；**Aptabase**（`tauri-plugin-aptabase`）負責隱私優先的匿名使用量分析（DAU/MAU、版本 / OS 分布、功能事件）。兩者職責分離，事件不含任何機密。

- **啟用條件** — 編譯期 `APTABASE_KEY`（fork 專屬 GitHub variable）存在才註冊 plugin；無 key → 不註冊 → 所有埋點自動 no-op（dev 安全）。`get_aptabase_key()` 在 `lib.rs`。
- **⚠️ 1.0.0 API 陷阱** — `EventTracker::track_event(name, Option<serde_json::Value>) -> Result<(),String>` / `flush_events_blocking()` 內部以 `state()` 取 client，**plugin 未註冊會 panic**；impl 為具體 `App/AppHandle/Window`（非泛型）。故 Rust helper 用具體 `AppHandle` 且送出前以 `aptabase_configured()`（key 是否存在）把關。見 `plugins/analytics.rs`。
- **Rust 埋點** — `app_started`（setup）、`app_exited` + `flush`（`_exit(0)` 前，與 Sentry flush 並列）。走 `EventTracker` trait，**不經** command / ACL。
- **前端埋點** — `src/lib/analytics.ts` 的 `trackEvent()` 走 `invoke("plugin:aptabase|track_event", { name, props })`（由 Rust plugin 以 reqwest 送出，**不受** CSP / http allowlist；但需 `capabilities/default.json` 加 `aptabase:allow-track-event`）。事件：`transcription_completed/failed`、`vocabulary_learned`、`provider_changed`、`screen_view`（router afterEach，Dashboard-only）。
- **opt-out（預設開）** — 設定 key `analyticsEnabled`（存 `tauri-plugin-store`）。Rust 端 `ANALYTICS_ENABLED` AtomicBool（setup 從 store 載入 + `set_analytics_enabled` command 即時切換）；前端 `analytics.ts` 快取 `enabled`（啟動讀 store + 監聽 `settings:updated`）。UI 開關 + 隱私說明在 `SettingsView`（五語系）。
- **隱私硬規則** — 事件屬性「只能」是匿名枚舉字串與數值（provider 名、語言碼、模式、字數 / 時長 / 計數）；**絕不**送轉錄 / LLM 文字、字典詞、金鑰 / 憑證、剪貼簿、欄位文字、路徑、主機名。`analytics.ts` 的 `trackEvent`／`initAnalytics` 以 try/catch 保證在無 Tauri（測試）環境不 throw、不產生 unhandled rejection。
