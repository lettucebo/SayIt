...
**Claude Code Review Workflow：** `.github/workflows/claude.yml`（`@claude` comment 觸發）+ `claude-code-review.yml`（PR 自動 review）。需安裝 Claude Code GitHub App + 設 `CLAUDE_CODE_OAUTH_TOKEN`。**Fork PR 硬規則**：`claude-code-review.yml` 的 job 必須保留 `if: github.event.pull_request.head.repo.full_name == github.repository` guard（**禁止移除**，否則 fork PR 因無 `id-token: write` 而紅 ❌；見 `docs/adr-claude-code-review-fork-pr.md`）。
...
5. **Commit message 禁止包含 `Co-authored-by` trailer** — 任何 commit 一律**不得**加入 `Co-authored-by`（含 Copilot/AI 署名）或其他 co-author 標記。
