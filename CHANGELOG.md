...
- 設定頁新增 MAI-Transcribe 的語音轉錄模型選擇，可在 1.5 與 2 之間切換。MAI-Transcribe 2 支援 60 種語言，抗噪與語碼切換表現較佳。兩代的請求格式並不相同：2 把逐字稿風格參數移到 `modelOptions` 之下，而且服務端預設從「可讀」翻轉成「逐字」，因此選「可讀性最佳化」時會明確送出 `clean`，避免整理前的逐字稿混入「嗯」「呃」等填充詞。輸入語言與逐字稿風格兩代共用同一組選項，格式差異全部在後端吸收。新安裝預設使用 2；已經在用 MAI 的使用者維持 1.5，輸出行為與升級前一致。

### Fixed

- Groq 的 Qwen3.6 27B 被無預警下架後，AI 整理階段回 404 而整段失敗的問題。它原本是預設模型，而且有五個更早的模型遷移對應到它，所以全新安裝與從舊版升級的使用者都會踩到；語音轉錄本身不受影響，只是少了書面語整理，容易被忽略。預設改用 Qwen3.8 27B（免費層每日 200 萬 tokens，是其他 Groq 模型的 10 倍），舊模型的遷移對應一併指向它。另外補上測試守住「預設模型必須存在於清單、且不得同時被列為已下架」，讓同類問題在發版前就會被擋下。
- 開發建置執行時不再搶走正式版的 Windows 開機自啟動設定。開機自啟動的登錄值名稱取自 productName、不含應用程式識別碼，所以所有 SayIt 建置共用同一個項目，先前開發建置首次啟動時會把它覆寫成帶主控台視窗的開發執行檔。現在會先判斷是否為 debug 建置，是的話跳過首次啟動的自動註冊，並改用專屬名稱；正式版行為不變。

## [1.1.1] - 2026-09-02

### Fixed

- Windows HUD 的透明背景在 DWM 合成狀態遺失後不再變成完整黑色方塊。HUD 顯示與滑鼠穿透現在由同一個原生生命週期操作管理，並在視窗樣式更新後重新套用透明合成、topmost 與不搶焦點屬性；圓弧 notch 造型因此能持續正確顯示。macOS 的透明視窗路徑維持不變。
...
- 模型與服務選擇器在視窗變窄時不再擠壓或溢出：固定側邊欄會讓可用寬度在桌面斷點突然縮小，現在選項會依視窗寬度切換一至三欄，較長的模型名稱與多語系文字也能完整呈現。

### Improved

- LLM 與語音轉錄服務更容易辨識：Groq 與 Gemini 改用本地化 Badge 標示免費服務，推薦星號移至預設的 Groq；OpenAI、Anthropic 與 Microsoft Foundry 維持無標記，減少選單中的文字與視覺噪音。

## [1.0.3] - 2026-08-27

### Improved

- 功能介紹頁現在可直接前往 SayIt 的 GitHub Releases 頁面查看版本更新說明，設定頁的外部連結也改為統一交給系統瀏覽器開啟：先前直接依賴 Tauri v2 WebView 的 `target="_blank"`，不同平台的開啟行為並不一致。現在透過共用外連元件處理一般與中鍵點擊，並由前端與 Tauri 設定雙層限制僅允許 HTTPS。
...
- Windows 系統匣圖示在淺色或深色工作列下可能看不清楚的問題：先前固定使用單一圖示，有些系統主題的前景與背景對比不足。現在會依 Windows 工作列主題自動選擇高對比的單色圖示；macOS 維持原本的 template icon 行為。
...
- 設定頁將 MAI-Transcribe 列為語音轉錄的首選項目，並標示品質最佳，不會自動變更你目前選用的轉錄服務。
