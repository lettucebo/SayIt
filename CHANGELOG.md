...
- 自動更新的背景檢查期間改為佔用「檢查中」狀態，避免與 Sidebar 手動「檢查更新」同時執行——兩者共用同一個待安裝更新物件，並行時可能把已下載好的更新換掉。
...
- 補正自動更新的文件敘述：多份文件寫「每 4 小時檢查」、程式碼實際是 15 分鐘，且位置標成 `main-window.ts`（實際在 `MainApp.vue`），現已一併更新為新行為；另移除一條早已不成立的「已知限制」（更新提示用 `window.confirm`，實際上流程早就改用 AlertDialog）。
