...
先動態取得 repo root（**不要寫死 clone 路徑**），後續所有命令都以它為基準：
...
讀取當前版本（**不依賴 `jq`**；Windows 用 PowerShell，macOS/Linux 用 Node，兩者都是本專案必備的工具）：

```powershell
# Windows
(Get-Content (Join-Path $Root 'src-tauri/tauri.conf.json') -Raw | ConvertFrom-Json).version
```
...
兩件事並行做（`-C "$ROOT"` / `-C $Root` 讓命令不受目前工作目錄影響）：

```powershell
# Windows
git -C $Root log "$(git -C $Root describe --tags --abbrev=0)..HEAD" --no-merges --pretty='%h %s'
...
日期用今天的日期，執行時動態取得、不要寫死：Windows 用 `Get-Date -Format 'yyyy-MM-dd'`，macOS/Linux 用 `date +%Y-%m-%d`。
...
實際呼叫發版腳本之前確認三件事，不對就回頭修。`$Root` / `$ROOT` 沿用步驟 ① 的動態 repo root，`$N` 是本版 item 數：

```powershell
# Windows（主要路徑）— 只用內建 cmdlet，不需要 rg / jq
# 1. 5 個 .json 的 upgradeNotice 只有 title + item1..itemN + dismiss，且 key 集合一致
Get-ChildItem (Join-Path $Root 'src/i18n/locales') -Filter *.json | ForEach-Object {
  $keys = ((Get-Content $_.FullName -Raw | ConvertFrom-Json).mainApp.upgradeNotice.PSObject.Properties.Name | Sort-Object) -join ','
  "$($_.Name): $keys"
}

# 2. MainApp.vue 的 itemCount 等於 N
Select-String -Path (Join-Path $Root 'src/MainApp.vue') -Pattern 'upgradeNoticeItemCount = '

# 3. CHANGELOG.md 含 [X.Y.Z] 區塊
Select-String -Path (Join-Path $Root 'CHANGELOG.md') -Pattern '^## \[X\.Y\.Z\]'
```

```bash
# macOS / Linux — 用 node 取代 jq，用 grep 取代 rg
for f in "$ROOT"/src/i18n/locales/*.json; do
  node -p "const k=Object.keys(require('$f').mainApp.upgradeNotice).sort().join(','); '$(basename "$f"): '+k"
done
grep -n 'upgradeNoticeItemCount = ' "$ROOT/src/MainApp.vue"
grep -n '^## \[X\.Y\.Z\]' "$ROOT/CHANGELOG.md"
```

三項判準：5 個檔的 key 集合都等於 `dismiss,item1..itemN,title`、`upgradeNoticeItemCount` 等於 N、CHANGELOG 有目標版本區塊。任何一項對不上，回去把它修好再走步驟 ⑥。
...
9. Windows 先執行 `.\scripts\release.ps1 X.Y.Z -WhatIf`；macOS/Linux 確認 `jq`、`python3` 可用（**這兩個是 `release.sh` 自身的相依，Windows 路徑不需要**）。
...
```bash
cd "$ROOT" && ./scripts/release.sh X.Y.Z
```
...
CHANGELOG 標題的日期應該等於今天日期，不是亮點被開發的日期。執行時動態取得，不要寫死字串：Windows `Get-Date -Format 'yyyy-MM-dd'`、macOS/Linux `date +%Y-%m-%d`。

### 路徑一律動態解析

skill 內所有命令都以 `git rev-parse --show-toplevel` 取得的 repo root 為基準，**不得寫死任何本機 clone 路徑**（例如 `C:/Source/Repos/SayIt`）。這樣同一份 skill 在 worktree、其他 clone 位置與 macOS/Linux 都能用。
