...
- **E2E 用 `data-testid` selector**，勿用 CSS class（Tailwind 一改就壞）；`test`/`expect` 從 `tests/support/fixtures` import（fixtures 負責 auto-cleanup）。E2E 首次需 `npx playwright install chromium`。E2E 跑在 mock 過 Tauri 的 Vite dev server，**碰不到真 tauri-plugin-sql**；要驗真 DB/匯入須用 `pnpm tauri dev`。
