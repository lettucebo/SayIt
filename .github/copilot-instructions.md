...
- **Chat（LLM 整理）** — provider `"azure"`，走 Azure OpenAI v1 端點 `{endpoint}/openai/v1/chat/completions`（OpenAI 線相容，同路徑可接 Foundry Azure 直售的 DeepSeek、Kimi、Grok、Llama、Mistral、Cohere 等部署）。`model` 一律是使用者自訂的部署名稱。若使用者貼上完整 Foundry 專案端點 `{origin}/api/projects/{project}`，`foundryDeployments.ts` 會以同一個 data-plane host 與 `ai.azure.com/.default` scope 呼叫 `/deployments?api-version=v1`，取得 deployment name、publisher、model name、version 與 capabilities，並只列出 `chat=true` 的模型；無 project 或呼叫失敗時才降級 `/openai/v1/models` 名稱清單。SettingsView 會以清單為主要入口：使用者明確選擇帶 Foundry metadata 的部署時，原子寫入 deployment、family 與 `azureChatModelFamilySource="auto"`；手動改 family 或手動輸入部署時標為 `"manual"`。重新載入清單不可改寫已存 family；舊設定沒有 source 時一律視為 manual；已存但不在清單的 deployment 要以虛擬選項保留。名稱猜測只能顯示建議，絕不可自動持久化。`azureChatModelFamily` 是可覆寫的 wire 行為提示；未知 metadata 落保守 `other`／`other-reasoning` profile。未知／誤判不應造成請求失敗：初始 token 預設保守為 8192，明確的 HTTP 400 temperature 拒絕會移除該參數重試一次並以 endpoint+deployment 快取結論（快取不可備份）。`buildFetchParams("azure", …, azureOptions)` 在 `llmProvider.ts`。不要改用已退役的 `/models/chat/completions` 路由，也不要對非 OpenAI 模型送未經文件確認的 `reasoning_effort`。
